...
int
test_authtoken(const char* authUser,
...
main(int argc, char** argv)
{
  ...
  int Base64Encode(
    const unsigned char* buffer, const size_t length, char** b64text);
  int Base64Decode(
    const char* b64message, unsigned char** buffer, size_t* length);
  ...
  assert(Base64Encode((unsigned char*)base64String,
                      strlen(base64String),
                      &base64Text) == 0);
  assert(strcmp(base64Text, base64EncodeCheck) == 0);
  assert(Base64Decode(base64Text,
               ...
               EVP_PKEY* privkey)
{
  ...
  assert(encode_auth_setting(
           authUser, authPassword, pubkey, &authToken, use_pkcs1_padding) == 0);
  assert(decode_auth_setting(0,
                             authToken,
                             privkey,
                             &decodeUser,
                             &decodePassword,
