...
/* Workaround for `pthread_cancel()` in Android, using `pthread_kill()`
 * instead, as Android NDK does not support `pthread_cancel()`.
 */
...
void
iperf_thread_exit_handler(int sig)
{
  pthread_exit(0);
}

int
iperf_set_thread_exit_handler()
{
  int rc;
