...
#define SERVER_ERROR (-2)
  ...
  void iperf_set_test_socket_bufsize(struct iperf_test* ipt,
                                     int socket_bufsize);
  ...
  void iperf_set_test_template(struct iperf_test* ipt,
  ...
  void save_tcpinfo(struct iperf_stream* sp,
  ...
  enum
  {
    ...
    IERECVMESSAGE = 112, // Unable to receive control message from
                         // client/server (check perror)
    ...
    IEHOSTDEV = 147,          // host device name (ip%%<dev>) is supported (and
                              // required) only for IPv6 link-local address
