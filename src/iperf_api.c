...
static void
print_interval_results(struct iperf_test* test,
...
usage_long(FILE* f)
{
  fprintf(f,
          usage_longstr,
          DEFAULT_NO_MSG_RCVD_TIMEOUT,
          UDP_RATE / (1024 * 1024),
          DEFAULT_PACING_TIMER,
          DURATION,
          DEFAULT_TCP_BLKSIZE / 1024,
          DEFAULT_UDP_BLKSIZE);
}
...
void
iperf_set_test_reporter_interval(struct iperf_test* ipt,
...
void
iperf_set_test_bitrate_limit_maximum(struct iperf_test* ipt,
                                     uint64_t total_rate)
{
...
void
iperf_set_test_bitrate_limit_interval(struct iperf_test* ipt,
...
iperf_set_test_bitrate_limit_stats_per_interval(
  struct iperf_test* ipt,
  uint64_t bitrate_limit_stats_per_interval)
{
...
void
iperf_set_test_server_hostname(struct iperf_test* ipt,
...
#if defined(HAVE_SSL)
void
iperf_set_test_client_username(struct iperf_test* ipt,
...
void
iperf_set_test_client_password(struct iperf_test* ipt,
...
void
iperf_set_test_client_rsa_pubkey(struct iperf_test* ipt,
...
void
iperf_set_test_server_authorized_users(struct iperf_test* ipt,
...
void
iperf_set_test_server_skew_threshold(struct iperf_test* ipt,
...
void
iperf_set_test_server_rsa_privkey(struct iperf_test* ipt,
...
void
iperf_set_test_udp_counters_64bit(struct iperf_test* ipt,
...
iperf_on_connect(struct iperf_test* test)
{
  ...
  if (test->json_output)
    cJSON_AddItemToObject(
      test->json_start,
      "timestamp",
      iperf_json_printf("time: %s  timesecs: %d", now_str, (int64_t)now_secs));
  ...
  if (test->role == 'c') {
    if (test->json_output)
      ...
      iperf_printf(
        test, report_connecting, test->server_hostname, test->server_port);
      if (test->reverse)
    ...
    if (test->protocol->id == SOCK_STREAM) {
      if (test->settings->mss)
        cJSON_AddNumberToObject(
          test->json_start, "tcp_mss", test->settings->mss);
      else {
        cJSON_AddNumberToObject(
          test->json_start, "tcp_mss_default", test->ctrl_sck_mss);
      }
    }
    // Duplicate to make sure it appears on all output
    cJSON_AddNumberToObject(
      test->json_start, "target_bitrate", test->settings->rate);
    cJSON_AddNumberToObject(
      test->json_start, "fq_rate", test->settings->fqrate);
  } else if (test->verbose) {
...
iperf_parse_arguments(struct iperf_test* test, int argc, char** argv)
{
  ...
  */
  if (test->settings->bytes == 0 && test->settings->blocks == 0 &&
    ...
    // if failing to read file stat, it should fallback to default duration
    // mode
  }
 ...
 */
int
iperf_open_logfile(struct iperf_test* test)
{
...
void
iperf_close_logfile(struct iperf_test* test)
{
...
void
iperf_check_total_rate(struct iperf_test* test,
                       iperf_size_t last_interval_bytes_transferred)
{
  ...
  test->bitrate_limit_intervals_traffic_bytes
    [test->bitrate_limit_last_interval_index] = last_interval_bytes_transferred;
  ...
  for (i = 0, total_bytes = 0;
       i < test->settings->bitrate_limit_stats_per_interval;
       i++) {
  ...
  if (test->debug) {
    iperf_printf(test,
                 ...
                 " bps (limit %" PRIu64 ")\n",
                 test->bitrate_limit_stats_count,
                 bits_per_second,
...
iperf_create_send_timers(struct iperf_test* test)
{
  ...
  SLIST_FOREACH(sp, &test->streams, streams)
  {
    ...
    if (test->settings->rate != 0 && sp->sender) {
      cd.p = sp;
      sp->send_timer =
        tmr_create(NULL, send_timer_proc, cd, test->settings->pacing_timer, 1);
...
#if defined(HAVE_SSL)
int
test_is_authorized(struct iperf_test* test)
{
  ...
  if (test->settings->authtoken) {
    ...
    int ret = check_authentication(username,
                                   password,
                                   ts,
...
send_parameters(struct iperf_test* test)
{
  ...
  if (j == NULL) {
    ...
    if (test->get_server_output)
      cJSON_AddNumberToObject(
        j, "get_server_output", iperf_get_test_get_server_output(test));
    if (test->udp_counters_64bit)
      cJSON_AddNumberToObject(
        j, "udp_counters_64bit", iperf_get_test_udp_counters_64bit(test));
  ...
  if (j == NULL) {
    ...
    if (test->role == 's' && test->get_server_output) {
      if (test->json_output) {
        ...
        /* Figure out how much room we need to hold the complete output
         * string */
    ...
    if (j_streams == NULL) {
      ...
      SLIST_FOREACH(sp, &test->streams, streams)
      {
        ...
        if (j_stream == NULL) {
          ...
          cJSON_AddNumberToObject(
            j_stream, "omitted_errors", sp->omitted_cnt_error);
          cJSON_AddNumberToObject(j_stream, "packets", sp->packet_count);
          cJSON_AddNumberToObject(
            j_stream, "omitted_packets", sp->omitted_packet_count);

          iperf_time_diff(
            &sp->result->start_time, &sp->result->start_time, &temp_time);
          start_time = iperf_time_in_secs(&temp_time);
          iperf_time_diff(
            &sp->result->start_time, &sp->result->end_time, &temp_time);
    ...
    if (j_cpu_util_total == NULL || j_cpu_util_user == NULL ||
      ...
      if (j_streams == NULL) {
            ...
            if (j_id == NULL || j_bytes == NULL || j_retransmits == NULL ||
              ...
              r = -1;
                       ...
                       (j_omitted_errors != NULL &&
                        j_omitted_packets == NULL)) {
              /* For backward compatibility allow to not receive
               * "omitted" statistcs */
              i_errno = IERECVRESULTS;
              r = -1;
            } else {
                ...
                if (sp->sender) {
                  ...
                  if (j_omitted_packets != NULL) {
                    ...
                    if (sp->peer_omitted_packet_count > 0) {
                      /* -1 indicates unknown error count
                       * since it includes the omitted
                       * count */
                   ...
                   * available; this is the case for older
                   * (pre-3.2) servers.
                   *
                   * We need to have result structure members
                   * to hold the both sides' start_time and
                   * end_time.
                   */
        ...
        if (test->role == 'c' && iperf_get_test_get_server_output(test)) {
          /* Look for JSON.  If we find it, grab the object so it
           * doesn't get deleted. */
          ...
          if (j_server_output != NULL) {
            ...
            /* No JSON, look for textual output.  Make a copy of the
             * text for later. */
...
void
add_to_interval_list(struct iperf_stream_result* rp,
...
connect_msg(struct iperf_stream* sp)
{
  ...
  if (getsockdomain(sp->socket) == AF_INET) {
    inet_ntop(AF_INET,
              (void*)&((struct sockaddr_in*)&sp->local_addr)->sin_addr,
              ipl,
              sizeof(ipl));
  ...
  if (sp->test->json_output)
    ...
    iperf_printf(
      sp->test, report_connected, sp->socket, ipl, lport, ipr, rport);
}
...
iperf_new_test()
{
  ...
  memset(test->bitrate_limit_intervals_traffic_bytes,
         0,
...
iperf_stats_callback(struct iperf_test* test)
{
  ...
  SLIST_FOREACH(sp, &test->streams, streams)
  {
    ...
    if (irp != NULL) /* not the 1st interval */
      memcpy(
        &temp.interval_start_time, &rp->end_time, sizeof(struct iperf_time));
    else /* or use timestamp from beginning */
      memcpy(
        &temp.interval_start_time, &rp->start_time, sizeof(struct iperf_time));
    ...
    iperf_time_diff(
      &temp.interval_start_time, &temp.interval_end_time, &temp_time);
  ...
  for (current_mode = lower_mode; current_mode <= upper_mode; ++current_mode) {
    ...
    SLIST_FOREACH(sp, &test->streams, streams)
    {
      if (sp->sender == stream_must_be_sender) {
        ...
        if (irp == NULL) {
          iperf_err(test,
                    "iperf_print_intermediate error: "
                    "interval_results is NULL");
          return;
        }
    ...
    if (test->num_streams > 1 || test->json_output) {
      /*
       * With BIDIR give a different JSON object name to the one
       * sent/receive sums. The different name is given to the data sent
       * from the server, which is the "reverse" channel.  This makes sure
       * that the name reported on the server and client are compatible,
       * and the names are the same as with non-bidir, except for when
       * reverse is used.
       */
      sum_name = "sum";
      if (test->mode == BIDIRECTIONAL) {
      ...
      if (sp) {
        ...
        iperf_time_diff(
          &sp->result->start_time, &irp->interval_start_time, &temp_time);
        start_time = iperf_time_in_secs(&temp_time);
        iperf_time_diff(
          &sp->result->start_time, &irp->interval_end_time, &temp_time);
        end_time = iperf_time_in_secs(&temp_time);
            ...
            if (test->json_output)
              cJSON_AddItemToObject(
                ...
                iperf_json_printf(
                  ...
                  "bits_per_second: %f  retransmits: %d  "
                  "omitted: %b sender: %b",
                  (double)start_time,
                  (double)end_time,
          ...
          if (stream_must_be_sender) {
            if (test->json_output)
              cJSON_AddItemToObject(
                ...
                sum_name,
                iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  "
                                  "bits_per_second: %f  packets: %d  omitted: "
                                  "%b sender: %b",
                                  (double)start_time,
                                  (double)end_time,
                                  (double)irp->interval_duration,
            ...
            if (test->json_output)
              cJSON_AddItemToObject(
                ...
                iperf_json_printf(
                  "start: %f  end: %f  seconds: %f  bytes: %d  "
                  "bits_per_second: %f  jitter_ms: %f  "
                  "lost_packets: %d  packets: %d  lost_percent: "
                  "%f  omitted: %b sender: %b",
                  (double)start_time,
                  (double)end_time,
                  (double)irp->interval_duration,
                  (int64_t)bytes,
  ...
  for (current_mode = lower_mode; current_mode <= upper_mode; ++current_mode) {
      ...
      SLIST_FOREACH(sp, &test->streams, streams)
      {
        if (sp->sender == stream_must_be_sender) {
          ...
          if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
            ...
            /*
             * Running total of the total number of packets.  Use the
             * sender packet count if we have it, otherwise use the
             * receiver packet count.
             */
            int64_t packet_count =
              sender_packet_count ? sender_packet_count : receiver_packet_count;
          ...
          if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
            if (test->sender_has_retransmits) {
              ...
              if (test->json_output)
                cJSON_AddItemToObject(
                  ...
                  report_sender,
                  iperf_json_printf(
                    "socket: %d  start: %f  end: %f  seconds: "
                    "%f  bytes: %d  bits_per_second: %f  "
                    "retransmits: %d  max_snd_cwnd:  %d  "
                    "max_snd_wnd:  %d  max_rtt:  %d  min_rtt:  "
                    "%d  mean_rtt:  %d sender: %b",
                    (int64_t)sp->socket,
                    (double)start_time,
                    (double)sender_time,
                    (double)sender_time,
                ...
                if (test->verbose)
                  iperf_printf(
                    test, report_sender_not_available_format, sp->socket);
              } else {
                iperf_printf(test,
              ...
              if (test->json_output)
                cJSON_AddItemToObject(
                  ...
                  iperf_json_printf(
                    "socket: %d  start: %f  end: %f  seconds: "
                    "%f  bytes: %d  bits_per_second: %f sender: "
                    "%b",
                    (int64_t)sp->socket,
                    (double)start_time,
                    (double)sender_time,
            ...
            if (test->json_output) {
               ...
               * side.
               *
               * The JSON format as currently defined only includes
               * one value for the number of packets.  We usually
               * want that to be the sender's value (how many packets
               * were sent by the sender).  However this value might
               * not be available on the receiver in certain
               * circumstances specifically on the server side for a
               * normal test or the client side for a reverse-mode
               * test.  If this is the case, then use the receiver's
               * count of packets instead.
               */
              ...
              cJSON_AddItemToObject(
                ...
                "udp",
                iperf_json_printf(
                  "socket: %d  start: %f  end: %f  seconds: %f  "
                  "bytes: %d  bits_per_second: %f  jitter_ms: "
                  "%f  lost_packets: %d  packets: %d  "
                  "lost_percent: %f  out_of_order: %d sender: "
                  "%b",
                  (int64_t)sp->socket,
                  (double)start_time,
                  (double)sender_time,
          ...
          if (sp->diskfile_fd >= 0) {
            if (fstat(sp->diskfile_fd, &sb) == 0) {
              /* In the odd case that it's a zero-sized file, say it
               * was all transferred. */
              ...
              if (sb.st_size > 0) {
                ...
                percent_received =
                  (int)(((double)bytes_received / (double)sb.st_size) * 100.0);
              }
              ...
              if (test->json_output)
                cJSON_AddItemToObject(
                  json_summary_stream,
                  "diskfile",
                  iperf_json_printf("sent: %d  received: %d  size: %d  "
                                    "percent_sent: %d  percent_received: %d  "
                                    "filename: %s",
                                    (int64_t)bytes_sent,
          ...
          if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
            ...
            if (test->json_output)
              cJSON_AddItemToObject(
                ...
                iperf_json_printf(
                  "socket: %d  start: %f  end: %f  seconds: %f  "
                  "bytes: %d  bits_per_second: %f sender: %b",
                  (int64_t)sp->socket,
                  (double)start_time,
              ...
              if (test->verbose)
                iperf_printf(
                  test, report_receiver_not_available_format, sp->socket);
            } else {
              iperf_printf(test,
                           report_bw_format,
                           sp->socket,
      ...
      if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
        if (test->sender_has_retransmits) {
          ...
          if (test->json_output)
            ...
            if (test->verbose)
              iperf_printf(
                test, report_sender_not_available_summary_format, "SUM");
          } else {
            iperf_printf(test,
                         report_sum_bw_retrans_format,
                         mbuf,
        ...
        if (test->json_output)
          ...
          if (test->verbose)
            iperf_printf(
              test, report_receiver_not_available_summary_format, "SUM");
        } else {
          iperf_printf(test,
                       report_sum_bw_format,
                       mbuf,
        ...
        if (test->json_output) {
          ...
          cJSON_AddItemToObject(
            ...
            iperf_json_printf(
              "start: %f  end: %f  seconds: %f  bytes: %d  "
              "bits_per_second: %f  jitter_ms: %f  lost_packets: "
              "%d  packets: %d  lost_percent: %f sender: %b",
              (double)start_time,
              (double)receiver_time,
              (double)receiver_time,
              (int64_t)total_sent,
          ...
          cJSON_AddItemToObject(
            ...
            iperf_json_printf(
              ...
              (double)sender_time,
              (int64_t)total_sent,
              (double)total_sent * 8 / sender_time,
              (double)0.0,
              (int64_t)0,
              (int64_t)sender_total_packets,
              (double)0.0,
              1));
          cJSON_AddItemToObject(
            test->json_end,
          ...
          /*
           * On the client we have both sender and receiver overall
           * summary stats.  On the server we have only the side that
           * was on the server.  Output whatever we have.
           */
          ...
          if (!(test->role == 's' && stream_must_be_sender)) {
            ...
            unit_snprintf(
              nbuf, UNIT_LEN, bandwidth, test->settings->unit_format);
            iperf_printf(test,
                         report_sum_bw_udp_format,
                         mbuf,
                         start_time,
    ...
    if (test->json_output && current_mode == upper_mode) {
      cJSON_AddItemToObject(
        ...
        iperf_json_printf(
          "host_total: %f  host_user: %f  host_system: %f  "
          "remote_total: %f  remote_user: %f  remote_system: %f",
      ...
      if (test->protocol->id == Ptcp) {
        ...
        if (snd_congestion) {
          cJSON_AddStringToObject(
            test->json_end, "sender_tcp_congestion", snd_congestion);
        }
        if (rcv_congestion) {
          cJSON_AddStringToObject(
            test->json_end, "receiver_tcp_congestion", rcv_congestion);
        }
      }
    } else {
      if (test->verbose) {
                       ...
                       cJSON* json_interval_streams)
{
  ...
  if (test->mode == BIDIRECTIONAL) {
    sprintf(
      mbuf, "[%s-%s]", sp->sender ? "TX" : "RX", test->role == 'c' ? "C" : "S");
  } else {
    mbuf[0] = '\0';
    zbuf[0] = '\0';
  }
      ...
      if (test->json_output)
        cJSON_AddItemToArray(
          ...
          iperf_json_printf(
            ...
            "bits_per_second: %f  retransmits: %d  snd_cwnd:  %d  "
            "snd_wnd:  %d  rtt:  %d  rttvar: %d  pmtu: %d  omitted: "
            "%b sender: %b",
            (int64_t)sp->socket,
            (double)st,
            (double)et,
...
iperf_new_stream(struct iperf_test* test, int s, int sender)
{
  ...
  if (test->tmp_template) {
    snprintf(
      template, sizeof(template) / sizeof(char), "%s", test->tmp_template);
  } else {
    ...
    snprintf(
      template, sizeof(template) / sizeof(char), "%s/iperf3.XXXXXX", tempdir);
  }
...
iperf_init_stream(struct iperf_stream* sp, struct iperf_test* test)
{
  ...
  if (iperf_get_test_protocol_id(test) == Pudp &&
    ...
    /*
     * There are multiple implementations of this feature depending on the
     * OS. We need to handle separately Linux, UNIX, and Windows, as well as
...
iperf_json_finish(struct iperf_test* test)
{
  if (test->json_top) {
    ...
    if (test->json_server_output) {
      cJSON_AddItemToObject(
        test->json_top, "server_output_json", test->json_server_output);
    }
    if (test->server_output_text) {
      cJSON_AddStringToObject(
        test->json_top, "server_output_text", test->server_output_text);
    }
...
iperf_printf(struct iperf_test* test, const char* format, ...)
{
  ...
  if (iperf_get_test_timestamps(test)) {
    time(&now);
    ltm = localtime(&now);
    strftime(iperf_timestr,
             sizeof(iperf_timestr),
             iperf_get_test_timestamp_format(test),
             ltm);
    ct = iperf_timestr;
  }
