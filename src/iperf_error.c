...
iperf_err(struct iperf_test* test, const char* format, ...)
{
  ...
  if (test != NULL && test->timestamps) {
    ...
    ltm = localtime(&now);
    strftime(
      iperf_timestrerr, sizeof(iperf_timestrerr), test->timestamp_format, ltm);
    ct = iperf_timestrerr;
  }
  ...
  if (test != NULL && test->json_output && test->json_top != NULL)
    ...
    if (test && pthread_mutex_lock(&(test->print_mutex)) != 0) {
    ...
    if (test && pthread_mutex_unlock(&(test->print_mutex)) != 0) {
...
iperf_strerror(int int_errno)
{
  ...
  switch (int_errno) {
    ...
    case IEDURATION:
      snprintf(errstr,
               len,
               "test duration valid values are 0 to %d seconds",
               MAX_TIME);
      break;
    case IENUMSTREAMS:
      snprintf(errstr,
    ...
    case IEBLOCKSIZE:
      snprintf(errstr,
               len,
               "block size too large (maximum = %d bytes)",
               MAX_BLOCKSIZE);
      break;
    case IEBUFSIZE:
      snprintf(errstr,
    ...
    case IEBADTOS:
      snprintf(
        errstr, len, "bad TOS value (must be between 0 and 255 inclusive)");
      break;
    case IESETCLIENTAUTH:
      snprintf(errstr,
               len,
               "you must specify a username, password, and path to a valid "
               "RSA public key");
      break;
    case IESETSERVERAUTH:
      snprintf(errstr,
               len,
               "you must specify a path to a valid RSA private key and a "
    ...
    case IEBADPORT:
      snprintf(
        errstr, len, "port number must be between 1 and 65535 inclusive");
      break;
    case IEMSS:
    ...
    case IEUNIMP:
      snprintf(
        errstr, len, "an option you are trying to set is not implemented yet");
      break;
    case IEFILE:
    ...
    case IECONNECT:
      snprintf(errstr,
               len,
               "unable to connect to server - server may have stopped "
    ...
    case IEMESSAGE:
      snprintf(errstr,
               len,
               "received an unknown control message (ensure other side is "
               "iperf3 and not iperf)");
      break;
    case IESENDMESSAGE:
      snprintf(errstr,
               len,
    ...
    case IERECVMESSAGE:
      snprintf(errstr,
               len,
               "unable to receive control message - port may not be "
    ...
    case IEACCESSDENIED:
      snprintf(
        errstr, len, "the server is busy running a test. try again later");
      break;
    case IESETNODELAY:
    ...
    case IERCVTIMEOUT:
      snprintf(
        errstr, len, "receive timeout value is incorrect or not in range");
      perr = 1;
      break;
    case IESNDTIMEOUT:
    ...
    case IERVRSONLYRCVTIMEOUT:
      snprintf(
        errstr, len, "client receive timeout is valid only in receiving mode");
      perr = 1;
      break;
    case IEDAEMON:
    ...
    case IETOTALRATE:
      snprintf(
        errstr, len, "total required bandwidth is larger than server limit");
      break;
    ...
    case IEIDLETIMEOUT:
      snprintf(errstr,
               len,
               "idle timeout parameter is not positive or larger than "
               "allowed limit");
      break;
    case IEBINDDEV:
      snprintf(errstr,
    ...
    case IEBINDDEVNOSUPPORT:
      snprintf(errstr,
               len,
               "`<ip>%%<dev>` is not supported as system does not support "
               "bind to device");
      break;
    case IEHOSTDEV:
      snprintf(errstr,
               len,
               "host device name (ip%%<dev>) is supported (and required) "
