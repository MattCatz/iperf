...
int
check_authentication(const char* username,
                     const char* password,
                     const time_t ts,
                     const char* filename,
                     int skew_threshold)
...
int
Base64Encode(const unsigned char* buffer, const size_t length, char** b64text)
...
int
Base64Decode(const char* b64message, unsigned char** buffer, size_t* length)
...
EVP_PKEY*
load_pubkey_from_file(const char* file)
{
  BIO* key = NULL;
  EVP_PKEY* pkey = NULL;
...
EVP_PKEY*
load_pubkey_from_base64(const char* buffer)
{
...
EVP_PKEY*
load_privkey_from_file(const char* file)
{
  BIO* key = NULL;
  EVP_PKEY* pkey = NULL;
...
EVP_PKEY*
load_privkey_from_base64(const char* buffer)
{
  unsigned char* key = NULL;
  size_t key_len;
  Base64Decode(buffer, &key, &key_len);
...
int
test_load_pubkey_from_file(const char* file)
{
...
int
test_load_private_key_from_file(const char* file)
{
...
int
encrypt_rsa_message(const char* plaintext,
                    EVP_PKEY* public_key,
                    ...
                    int use_pkcs1_padding)
{
  ...
  rc = EVP_PKEY_get_int_param(public_key,
  ...
  *encryptedtext = (unsigned char*)OPENSSL_malloc(keysize);
  ...
  EVP_PKEY_encrypt(
    ctx, *encryptedtext, &encryptedtext_len, rsa_buffer, rsa_buffer_len);
  EVP_PKEY_CTX_free(ctx);
#else
  encryptedtext_len = RSA_public_encrypt(
    rsa_buffer_len, rsa_buffer, *encryptedtext, rsa, padding);
  RSA_free(rsa);
#endif
...
int
decrypt_rsa_message(const unsigned char* encryptedtext,
                    const int encryptedtext_len,
                    ...
                    int use_pkcs1_padding)
{
  ...
  /* See evp_pkey_rsa(7) and provider-keymgmt(7) */
  rc = EVP_PKEY_get_int_param(private_key,
                              OSSL_PKEY_PARAM_MAX_SIZE,
                              &keysize); /* XXX not really keysize */
  if (!rc) {
    goto errreturn;
...
#else
  plaintext_len =
    RSA_private_decrypt(rsa_buffer_len, rsa_buffer, *plaintext, rsa, padding);
  RSA_free(rsa);
#endif
...
int
encode_auth_setting(const char* username,
                    const char* password,
                    EVP_PKEY* public_key,
                    char** authtoken,
                    int use_pkcs1_padding)
{
  ...
  snprintf(
    text, text_len, auth_text_format, username, password, (int64_t)utc_seconds);

  unsigned char* encrypted = NULL;
  int encrypted_len;
  encrypted_len =
    encrypt_rsa_message(text, public_key, &encrypted, use_pkcs1_padding);
  free(text);
  if (encrypted_len < 0) {
    return -1;
  }
...
int
decode_auth_setting(int enable_debug,
                    ...
                    int use_pkcs1_padding)
{
  ...
  int plaintext_len;
  plaintext_len = decrypt_rsa_message(encrypted_b64,
                                      encrypted_len_b64,
                                      private_key,
                                      &plaintext,
                                      use_pkcs1_padding);
  ...
  int rc = sscanf(
    (char*)plaintext, auth_text_format, s_username, s_password, &utc_seconds);
  if (rc != 3) {
    free(s_password);
    free(s_username);
    return -1;
...
ssize_t
iperf_getpass(char** lineptr, size_t* n, FILE* stream)
{
