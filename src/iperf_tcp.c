...
iperf_tcp_send(struct iperf_stream* sp)
{
  ...
  if (sp->test->zerocopy)
    r = Nsendfile(sp->buffer_fd, sp->socket, sp->pending_size);
  else
    r = Nwrite(sp->socket, sp->buffer, sp->pending_size);
...
iperf_tcp_connect(struct iperf_test* test)
{
  ...
  s = create_socket(test->settings->domain,
                    SOCK_STREAM,
                    test->bind_address,
                    test->bind_dev,
                    test->bind_port,
  ...
  if (connect(
        s, (struct sockaddr*)server_res->ai_addr, server_res->ai_addrlen) < 0 &&
      errno != EINPROGRESS) {
    saved_errno = errno;
    close(s);
    freeaddrinfo(server_res);
