 ...
 * this by including "iperf.h", but net.c lives "below" this layer.  Clearly
 * the presence of this declaration is a sign we need to revisit this layering.
 */
extern int gerror;
...
create_socket(int domain,
              ...
              const char* server,
              int port,
              struct addrinfo** server_res_out)
{
        ...
        int timeout)
{
  ...
  s = create_socket(
    domain, proto, local, bind_dev, local_port, server, port, &server_res);
  if (s < 0) {
    return -1;
  }

  if (timeout_connect(s,
                      (struct sockaddr*)server_res->ai_addr,
                      server_res->ai_addrlen,
                      timeout) < 0 &&
      errno != EINPROGRESS) {
    saved_errno = errno;
    close(s);
...
Nsendfile(int fromfd, int tofd, size_t count)
{
  ...
  (defined(__APPLE__) && defined(__MACH__) && defined(MAC_OS_X_VERSION_10_6))
  off_t sent;
#endif
