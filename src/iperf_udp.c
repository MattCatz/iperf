...
iperf_udp_recv(struct iperf_stream* sp)
{
  ...
  if (sp->test->state == TEST_RUNNING) {
    ...
    if (sp->test->debug_level >= DEBUG_LEVEL_DEBUG)
      fprintf(stderr,
              "pcount %" PRIu64 " packet_count %" PRIu64 "\n",
              pcount,
              sp->packet_count);
...
iperf_udp_send(struct iperf_stream* sp)
{
  ...
  if (r <= 0) {
    --sp->packet_count; /* Don't count messages that no data was sent from
                         * them. Allows "resending" a massage with the same
                         * numbering */
    if (r < 0) {
  ...
  if (test->json_output) {
    ...
    if (sock_bufsize_item == NULL) {
      cJSON_AddNumberToObject(
        test->json_start, "sock_bufsize", test->settings->socket_bufsize);
    }
...
iperf_udp_accept(struct iperf_test* test)
{
   ...
   */
  len = sizeof(sa_peer);
  if ((sz = recvfrom(test->prot_listener,
                     &buf,
                     sizeof(buf),
                     0,
  ...
  if (rc > 0) {
    if (test->settings->socket_bufsize == 0) {
      ...
      snprintf(
        str, sizeof(str), "Increasing socket buffer size to %d", bufsize);
      warning(str);
  ...
  if (test->settings->fqrate) {
    ...
    if (fqrate > 0) {
      ...
      if (setsockopt(
            s, SOL_SOCKET, SO_MAX_PACING_RATE, &fqrate, sizeof(fqrate)) < 0) {
   ...
   */
  test->prot_listener = netannounce(test->settings->domain,
                                    Pudp,
                                    test->bind_address,
                                    test->bind_dev,
                                    test->server_port);
...
iperf_udp_connect(struct iperf_test* test)
{
  ...
  if ((s = netdial(test->settings->domain,
                   Pudp,
                   test->bind_address,
                   test->bind_dev,
                   test->bind_port,
  ...
  tv.tv_usec = 0;
  setsockopt(
    s, SOL_SOCKET, SO_RCVTIMEO, (struct timeval*)&tv, sizeof(struct timeval));
#endif
  ...
  if (test->reverse) /* In reverse mode allow few packets to have the "accept"
  ...
  do {
    ...
    if (test->debug) {
      printf("Connect received for Socket %d, sz=%d, buf=%x, i=%d, "
             "max_len_wait_for_reply=%d\n",
             s,
             sz,
             buf,
             i,
